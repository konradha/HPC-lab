function handle = gplotpart_proj(A,xy, zbounds,part1,color1,color2,color3)
...
if size(xy,2) == 2
    %fprintf("we here reprinting in 2d\n");
    axis([min(xy(:,1)) max(xy(:,1)) min(xy(:,2)) max(xy(:,2))]);
else
    %fprintf("we here axis in 3d\n");
    axis([min(xy(:,1)) max(xy(:,1)) min(xy(:,2)) max(xy(:,2)) ...
        min(zbounds) max(zbounds)]); 
end;
