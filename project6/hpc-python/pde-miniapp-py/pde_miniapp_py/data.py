...
class Field:
    ...
    def exchange_startall(self):
        ...
        if domain.neighbour_north >= 0:
            self._buffN = np.array([self.inner[:, -1]])
            self.req.append(domain.comm_cart.Isend(
                [
                    self._buffN, MPI.DOUBLE
                ],
                dest=domain.neighbour_north)
            )
            self.req.append(domain.comm_cart.Irecv(
                [
                    self._bdryN, MPI.DOUBLE
                ],
                source=domain.neighbour_north)
            )
        if domain.neighbour_south >= 0:
            self._buffS = np.array([self.inner[:, 0]])
            self.req.append(domain.comm_cart.Isend(
                [
                    self._buffS, MPI.DOUBLE
                ],
                dest=domain.neighbour_south)
            )
            self.req.append(domain.comm_cart.Irecv(
                [
                    self._bdryS, MPI.DOUBLE
                ],
                source=domain.neighbour_south)
            )
        if domain.neighbour_east >= 0:
            self._buffE = np.array([self.inner[-1, :]])
            self.req.append(domain.comm_cart.Isend(
                [
                    self._buffE, MPI.DOUBLE
                ],
                dest=domain.neighbour_east)
            )
            self.req.append(domain.comm_cart.Irecv(
                [
                    self._bdryE, MPI.DOUBLE
                ],
                source=domain.neighbour_east)
            )
        if domain.neighbour_west >= 0:
            self._buffW = np.array([self.inner[0, :]])
            self.req.append(domain.comm_cart.Isend(
                [
                    self._buffW, MPI.DOUBLE
                ],
                dest=domain.neighbour_west)
            )
            self.req.append(domain.comm_cart.Irecv(
                [
                    self._bdryW, MPI.DOUBLE
                ],
                source=domain.neighbour_west)
            )

    def exchange_waitall(self):
        """Wait until exchanging boundary field data is complete"""
        # ... implement ...
        stats = [MPI.Status() for _ in range(len(self.req))]
        MPI.Request.Waitall(self.req, stats)
